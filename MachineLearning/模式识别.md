...
$$
\begin{aligned}P(e) &= \int P(e|x) p(x) dx
\\~\\
...
要使错误率最小，因为错误率定义为
$$
P(error|\omega_i) = P(decide\  \omega_j | \omega_i) = \int_{R_j} p(\boldsymbol x | \omega_i) d \boldsymbol x  \ \ \ \ \ 其中i \neq j
$$
...
这里是ppt上的推导，比较繁琐，而且最后推论也是上面的二分类最小风险贝叶斯决策判据。为了好看用$\lambda_{ij}$表示$\lambda(a_i, \omega_j)$
